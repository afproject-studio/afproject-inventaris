...
// Fungsi untuk memeriksa apakah email sudah terdaftar di database admin
async function isEmailRegistered(email) {
    try {
        ...
        for (const username in users) {
            if (users[username].email_Admin === email) {
                return true;
            }
        }
        return false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Success modal login button (register)
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const adminSnapshot = await get(ref(db, 'admin'));
                ...
                if (adminSnapshot.exists()) {
                    ...
                    for (const username in admins) {
                        if (admins[username].email_Admin === email) {
                            adminFound = true;
                            adminData = admins[username];
                            console.log("✅ Admin found in database:", username);
                            break;
                        }
                    }
                    ...
                    if (!adminFound) {
                        for (const username in admins) {
                            if (admins[username].uid === user.uid) {
                                adminFound = true;
                                adminData = admins[username];
                                console.log("✅ Admin found by UID:", username);
                                
                                if (adminData.email_Admin !== email) {
                                    console.log("🔄 Updating email in database...");
                                    await update(ref(db, `admin/${username}`), {
                                        email_Admin: email,
                ...
                if (adminFound) {
                    ...
                    localStorage.setItem('adminData', JSON.stringify({
                        name: adminData.name_Admin,
                        email: email,
                        phone: adminData.nohp_Admin || '',
                        photo: adminData.foto_Admin || 'Logo/SIMIVA.png'
                    }));
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await set(ref(db, `admin/${username}`), data);
    ...
    inputFields.forEach(input => {
        if (input.id === 'iEmail' || input.id === 'iUsername' || input.id === 'iNama' || input.id === 'iPassword') {
